...
def probe_matrix(
    ir: EnsembleIR, X_train: FloatArray, n_random: int = 10_000, seed: int = 123
) -> FloatArray:
    ...
    train_part = X_train[rng.integers(0, len(X_train), size=min(len(X_train), 2000))]
    base_row = np.nan_to_num(X_train[0], nan=0.0)
    adjacent_part = threshold_adjacent_rows(ir, base_row)
    stacked = np.vstack([random_part, nan_part, train_part, adjacent_part])
...
def assert_conformance(
    ...
    X_eval = probe_matrix(ir, X_train, n_random=n_random)
