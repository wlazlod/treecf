...
This file, ``_exact_bounds``, ``_exact_domains``, ``_exact_orderpairs`` and
``_exact_propagation`` are one implementation, split across five files for
size alone. Together they are the Python reference for the exact backend; a
Rust mirror exists and matches all five bit-for-bit, so operation order in the
arithmetic is a compatibility contract, not a style choice. The score brackets
the search prunes on are re-summed in full over the trees in ascending index
after every assignment, never patched with an incremental delta — that
machinery lives in ``_exact_bounds`` — so the two implementations take the
same prune decisions and expand the same nodes. (The cost arithmetic lives in
``_exact_domains`` under the same contract.)
